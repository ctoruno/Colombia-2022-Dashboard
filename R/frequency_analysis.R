...
frequency_server <- function(id, filtered_data){
  ...
  moduleServer(
    ...
    function(input, output, session){
      ...
      twitter_tokenized.df <- input_data %>%
      ...
      wcount_flt.df <- wcount_raw.df %>%    # Removing keywords used to extract tweets
        ...
        { if (tags_switch == T) filter(.,!str_detect(words, "^@")) else . }
      
      # Creating wordcloud
      wordcloud <- wordcloud2(wcount_flt.df[1:200,], size = 0.9,
                              color = rep_len(c("DarkRed", "CornflowerBlue", "DarkOrange"),
                                              nrow(wcount_flt.df[1:200,])),
                              ellipticity = 0.2, shuffle = F)

      # Top words
      top_words <- datatable(wcount_flt.df %>%
                               rename(mentions = n))

      # Top hashtags
      top_hashtags <- datatable(wcount_flt.df %>%
